"""RC19-compatible fixed/random photo and video selection."""
...
class MediaPool:
    ...
    def from_settings(cls, settings: dict) -> "MediaPool":
        mode = str(settings.get("post_media_mode", "none")).strip().lower()
        ...
        if mode == "random":
            folder = settings.get("post_random_media_dir") or (Path.home() / "Desktop" / "view")
        ...
        fixed_value = str(settings.get("post_fixed_media_file") or "").strip()
        if not fixed_value:
